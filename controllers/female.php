...
if(isset($_GET["page"])){
    ...
    $sql = "SELECT * FROM upload_product WHERE gender = 'female' AND category = 'cloth' ORDER BY date DESC ";
    $result = Database::query("$sql LIMIT $offset, $per_page");
    if(!empty($result)){
        $p["perpage"] = showPerPage($sql,$per_page,$href);
    }
?>
    <div class="new-arrival-title best-sellers-title"><h4>Female Fashion</h4></div>
    <div class="fashion">
  ...
  <?php while($row = mysqli_fetch_assoc($result)){ ?>
    ...
    <div class="item-product" id="<?php echo $row["id"];?>">
        ...
        <div class="fashion-extra extra-div">
            <a href="quick.php?id=<?php echo $row["id"];?>">Add to Bag</a>
        </div>
    </div>
