...
if(isset($_GET["page"])){
    ...
    $sql = "SELECT * FROM upload_product WHERE gender = 'kid' AND category = 'cloth' ORDER BY date DESC ";
    $result = Database::query("$sql LIMIT $offset, $per_page");
    if(!empty($result)){
        $p["perpage"] = showPerPage($sql,$per_page,$href);
    }
?>
    <div class="new-arrival-title best-sellers-title"><h4>Kid Fashion</h4></div>
    <div class="fashion">
